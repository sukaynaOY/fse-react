...
const CliniqueTable = ({ data, searchQuery }) => {
      ...
      <table className="min-w-full">
        <thead className="bg-gradient-to-r bg-sky-800 to-blue-600">
          <tr>
            ...
            <th className="px-6 py-3 text-left text-sm font-semibold text-white uppercase">Nom Clinique</th>
            <th className="px-6 py-3 text-left text-sm font-semibold text-white uppercase">Address</th>
          </tr>
        </thead>
          ...
          {filteredData.length > 0 ? (
            filteredData.map((item) => (
              <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-6 py-4 text-sm text-gray-700">{item.nomClinique}</td>
